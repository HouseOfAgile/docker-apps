...
def generate_docker_app(myconfig):
    ...
    for app_config in myconfig["apps_inventory"]:
        ...
        docker_bases = (
            conf.load_config(app_config["docker_base"], "bases")
        ...
        if docker_bases:
            ...
            for variant in docker_bases["variants"]:
                ...
                docker_stack = update_parameters(docker_stack, myconfig["parameters"])
                docker_base = update_parameters(docker_base, myconfig["parameters"])
                for flavor in docker_flavors:
                    docker_flavors[flavor] = update_parameters(
                        docker_flavors[flavor], myconfig["parameters"]
                    )

                fu.generate_app_dockerfile(
                    path_app,
                    docker_stack,
                    docker_base,
                    docker_flavors,
        ...
        # copy files
        full_assembly_path = settings.current_path + os.path.join(
            os.path.sep, settings.assembly_path
        )

        fu.copy_files(full_assembly_path, "apps", app_config["name"], path_app)
        fu.copy_files(full_assembly_path, "bases", app_config["docker_base"], path_app)
